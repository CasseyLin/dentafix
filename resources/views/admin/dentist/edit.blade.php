...
<div class="row justify-content-center">
	...
	<div class="card-body">
		<form class="forms-sample" action="{{route('dentist.update',[$user->id])}}" method="post" enctype="multipart/form-data" >@csrf
            @method('PUT')
			<div class="row">
				<div class="col-lg-6">
					<label for="">Full name</label>
					<input type="text" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="dentist name" value="{{$user->name}}">
                    @error('name')
			...
			<div class="row">
				...
				<div class="row">
					<div class="col-lg-6">
						...
						<input type="text" name="education" class="form-control @error('education') is-invalid @enderror" placeholder="dentist highest degree" value="{{$user->education}}">
                         @error('education')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
					...
					<div class="col-lg-6">
						...
						<input type="text" name="address" class="form-control @error('address') is-invalid @enderror" placeholder="dentist address"  value="{{$user->address}}">
                         @error('address')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="service" class="form-control">
                            @foreach(App\Service::all() as $services)
                            <option value="{{$services->service}}" @if($user->service==$services->service)selected @endif>{{$services->service}}</option> 
                            @endforeach
                        </select>
                         @error('service')
                                    <span class="invalid-feedback" role="alert">
